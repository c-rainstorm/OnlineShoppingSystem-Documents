...
<!-- vscode-markdown-toc -->
* 1. [项目任务安排](#)
* 2. [09/05 - 09/18 MySQL](#MySQL)
* 3. [09/19 - 09/30 Markdown/Git](#MarkdownGit)
* 4. [10/10 - 10/19 JSP/servlet](#JSPservlet)
* 5. [10/20 - 10/26 HTML/CSS](#HTMLCSS)
* 6. [总体设计阶段任务分配](#-1)
* 7. [11/03 - 11/12 编码前任务安排](#-1)
	* 7.1. [学习任务安排](#-1)
	* 7.2. [系统分析与设计方面安排](#-1)
* 8. [编码阶段任务安排](#-1)
	* 8.1. [编码依据](#-1)
...
##  2. <a name='MySQL'></a>09/05 - 09/18 MySQL
...
##  3. <a name='MarkdownGit'></a>09/19 - 09/30 Markdown/Git
...
##  4. <a name='JSPservlet'></a>10/10 - 10/19 JSP/servlet
...
##  5. <a name='HTMLCSS'></a>10/20 - 10/26 HTML/CSS
...
##  7. <a name='-1'></a>11/03 - 11/12 编码前任务安排

###  7.1. <a name='-1'></a>学习任务安排
...
因为我们编码的时间比原计划又推迟了，所以我们的时间真的没剩多少了。

1. 每 2 - 3 天完成一个子模块，比如商品管理（包含增删改查），然后提交 PR。为保证质量我会进行 code review，所以提交以后请及时通知我。
1. PC 端编码需要在 12 月 3 号完成（从 11 月 24 号开始，一共 10 天）。大家把握好时间！**不允许把编码任务都推到最后几天**。

**Tips：不要为了赶时间随便写写。如果代码质量不过关，那么就需要重新修订代码了！**

###  8.3. <a name='-1'></a>其他

- 如果遇到了问题，**搜索半个小时**仍旧无法解决问题的话，请及时告诉我，我们一起解决！我之前在写点餐系统时，有时为填上一个坑会花 2 天的时间，就像无头苍蝇乱飞一样找不到关键的点在哪。但是现在不一样，我填过的那些坑你们就不用再填了，这样也能确保项目的顺利完成！**切记！**