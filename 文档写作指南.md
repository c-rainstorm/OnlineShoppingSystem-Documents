...
- 选用理由
    1. 它支持 Markdown 预览，并且可以手动配置任意的 Markdown 预览效果。为了能够看到 GitHub 上文档的效果，我们在本项目需要配置 github-markdown.css 来作为我们的 Markdown 预览效果，这样能很好的防止出现前后格式不一致的情况。
    2. 它内置 `git` 工具，方便将编辑后的文档，提交到 GitHub 。
    3. 后期编写代码时 VS Code 也是一个比较好的帮手。

- GitHub Markdown style 配置方法
...
文档结构（『`』号是为了防止生成目录时将其自动添加上而另外加上的，实际文档中并不需要）：
```
`# 文档标题
...
1. 在一个段落内禁止换行。在 VS Code 中的显示问题可以使用自动 wrap 来解决 view --> Toggle Word Wrap。这个地方如果有问题可以一起讨论。 
